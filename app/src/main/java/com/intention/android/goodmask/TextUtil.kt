...
internal object TextUtil {
    ...
    fun fromHexString(s: CharSequence): ByteArray {
        ...
        for (pos in 0 until s.length) {
            ...
            val c = s[pos].toInt().toByte()
            if(c.toInt() >= '0'.toInt() && c.toInt() <= '9'.toInt()) {
                nibble++
                (b.toInt() * 16).toByte()
                (b + (c.toInt() - '0'.toInt()).toByte()).toByte()
            }
            if (c >= 'A'.toInt() && c <= 'F'.toInt()) {
                nibble++
                (b * 16).toByte()
                (b + (c - 'A'.toInt() + 10).toByte()).toByte()
            }
            if (c >= 'a'.toInt() && c <= 'f'.toInt()) {
                nibble++
                (b * 16).toByte()
    ...
    fun toHexString(sb: StringBuilder, buf: ByteArray, begin: Int = 0, end: Int = buf.size) {
        for (pos in begin until end) {
            ...
            var c : Int = buf[pos].and(0xff.toByte()).toInt()/16
            if (c >= 10){
                 c = c + 'A'.toInt() - 10
             } else '0'.toInt()
            sb.append(c.toChar())
    ...
    internal class HexWatcher(private val s: String) : TextWatcher {
