...
import com.intention.android.goodmask.SerialSocket.*
...
class SerialSocket(context: Context, device: BluetoothDevice?) :
    ...
    fun connect(listener: SerialListener?) {
        Log.d("socket_connect", "socket connect")
        ...
        Log.d("socket_connect", "connect $device")
        context.registerReceiver(pairingBroadcastReceiver, pairingIntentFilter)
        gatt = if (Build.VERSION.SDK_INT < 23) {
            Log.d("socket_connect", "connectGatt")
            device!!.connectGatt(context, false, this)
        } else {
            Log.d("socket_connect", "connectGatt,LE")
    ...
    private fun onPairingBroadcastReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            BluetoothDevice.ACTION_PAIRING_REQUEST -> {
                ...
                Log.d(
                    TAG,
                    "pairing request $pairingVariant"
                )
            }
    ...
    private inner class TelitDelegate : DeviceDelegate() {
        ...
        override fun onCharacteristicWrite(
            ...
            if (characteristic === writeCreditsCharacteristic) { // NOPMD - test object identity
                Log.d(
                    TAG,
                    "write credits finished, status=$status"
                )
            }
        }
